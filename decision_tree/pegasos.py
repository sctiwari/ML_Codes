...
def objective_function(X, y, w, lamb):
    ...
    N=len(y)
    wtx= np.transpose(np.matmul(X,w))
    ywtx= np.multiply(y,wtx)
    loss= np.sum(np.maximum(0, 1-ywtx))/ N
    normw= np.linalg.norm(w,2)/2
    obj_value = (lamb* normw/2) + loss
    return obj_value
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        A_tx= []; A_ty=[]
        for index in A_t:
            wt= ytrain[index]* np.matmul(np.transpose(w), Xtrain[index])
            if wt < 1:
                A_tx.append(Xtrain[index])
                A_ty.append(ytrain[index])

        eta= float(1/(lamb*iter))
        yx= np.matmul(np.transpose(A_tx), A_ty)
        yetax= np.transpose(np.array([(eta/k)*yx]))
        wth= ((1- eta*lamb) *w) + yetax
        norm_wth= np.linalg.norm(wth,2)
        numr= 1/np.sqrt(lamb)
        normalize_wth= min(1, numr/norm_wth)
        w= normalize_wth* wth
